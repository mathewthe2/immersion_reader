...
file = os.path.join(directory, 'deinflect.json')
d = Deinflector(file)
# print(d.deinflect('出かけた', lambda term: [d['tags'] for d in '']))
def stringify_list(l):
    l = ["'" + a + "'" for a in l]
    return '[' + ', '.join(l) +']'

s = ''
for rule, variants in d.rules.items():
    s += "'" + rule + "': ["
    for v in variants:
        s += "DeinflectRule(kanaIn:'" + v['kanaIn'] + "', kanaOut:'" + v['kanaOut'] +  "', tagsIn:" +  stringify_list(v['tagsIn']) + ', tagsOut:' + stringify_list(v['tagsOut']) + ')'
        s += ','
    s += '],'
